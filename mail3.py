...
class Mailer():
    ...
    def _handle(self, user, name, msg_id, data):
        ...
        if msg.get('date') is None or msg.get('date').strip() == '':
            logger.info('date time is empty ,use default timestamp, today')
    ...
    def _load_meta(self):
        ...
        for f in os.listdir(mp):
            with open(mp + '/' + f, 'r') as ff:
                cache[f.replace('.meta', '').replace('--', '/')] = set([
    ...
    def _flush_meta(self, key=None):
        ...
        if key is not None:
            ...
            if value is None:
                return
            with open(mp + '/' + key.replace('/', '--') + '.meta', 'w+') as f:
